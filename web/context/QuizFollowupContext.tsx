...
export interface QuizFollowupController {
   ...
   */
  hydrateThread(questionKey: string, sessionId: string, messages: HydratedFollowupMessage[]): void
   ...
   */
  setOpenTabHandler(handler: ((ctx: QuizFollowupTabContext) => void) | null): void
  /**
...
export function QuizFollowupProvider({ children }: ProviderProps) {
  ...
  const runnersRef = useRef<Map<string, { questionKey: string; client: UnifiedWSClient }>>(
    new Map()
  )
  ...
  const openTabHandlerRef = useRef<((ctx: QuizFollowupTabContext) => void) | null>(null)
  ...
  const sendMessage = useCallback(
    (input: SendMessageInput) => {
      const current = threadsRef.current[input.questionKey] ?? createEmptyThreadState()
  ...
  const submitAskUserReply = useCallback(
    (
      ...
      const pendingAskUser = current.messages.some(m => hasPendingAskUser(m.events, turnId))
  ...
  return (
    <QuizFollowupCtx.Provider value={controller}>
      <QuizFollowupStateCtx.Provider value={threads}>{children}</QuizFollowupStateCtx.Provider>
...
export function useQuizFollowupController(): QuizFollowupController {
  ...
  if (!ctx) {
    throw new Error('useQuizFollowupController must be used inside a QuizFollowupProvider')
  }
  return ctx
}
