...
test("quiz follow-up preserves choice options for display and LLM context", () => {
  ...
  assert.deepEqual(
    (config.followup_question_context as { options: Record<string, string> })
      .options,
    question.options,
  );
});
...
test("voice exam reference resolves a persisted question without exposing it to GPT-Live", () => {
  ...
  assert.deepEqual(
    buildQuizFollowupConfig(referenced!, "", null, "exam-session"),
    {
      paper_id: "paper-123",
...
test("voice question reference supports Arabic and English ordinals", () => {
  ...
  assert.equal(
    findReferencedQuizQuestion("第十一題", elevenQuestions)?.question_id,
    "q-11",
  );
