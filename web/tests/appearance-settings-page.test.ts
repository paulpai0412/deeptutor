...
test('appearance settings page: adds the code blocks section after the theme section', () => {
  ...
  const themeIndex = source.search(/title=\{t\(["']Theme["']\)\}/)
  const codeBlocksIndex = source.search(/title=\{t\(["']Code blocks["']\)\}/)
  ...
  assert.ok(codeBlocksIndex > themeIndex, 'Code blocks section should come after Theme')
})
...
test('appearance settings page: preview includes a line long enough to demonstrate wrapping', () => {
  const source = readAppearancePage()
  const previewSource = source.match(/const CODE_BLOCK_PREVIEW_SNIPPET = `([\s\S]*?)`;?/)?.[1] ?? ''
