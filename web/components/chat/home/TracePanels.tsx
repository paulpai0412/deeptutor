...
type DisplayItem =
  { kind: 'trace'; trace: TraceItem } | { kind: 'step'; stepId: string; traces: TraceItem[] }
...
function isChatLoopAnswerContent(event: StreamEvent): boolean {
  return (
    event.type === 'content' && String(getTraceMeta(event).call_kind || '') === 'agent_loop_round'
  )
}
...
function buildDisplayItems(traceGroups: TraceItem[]): DisplayItem[] {
  ...
  for (const group of traceGroups) {
    ...
    // appear twice.
    if (group.events.some(e => getTraceMeta(e).absorbed_into_final === true)) continue
...
function LiveFoldRow({
  ...
  return (
    <div>
      <TraceRowHeader open={open} expandable active={active} onToggle={() => setUserOpen(!open)}>
        {summary}
      </TraceRowHeader>
      {open ? children : null}
    </div>
              ...
              <div className="space-y-0.5">
                {toolEvents.map((event, idx) => {
                  if (event.type === 'tool_call') {
                    const toolName = (event.metadata?.tool as string | undefined) ?? undefined
                    const niceArgs = renderNiceToolArgs(toolName, event.metadata?.args)
                    const formattedArgs = niceArgs ? '' : formatTraceArgs(event.metadata?.args)
                    return (
...
function TraceRowItem({
  ...
  const descriptor = isToolRow && toolName ? describeToolCall(toolName, toolArgs, t) : null
  ...
  if (descriptor) {
    ...
    headline = [thoughtText, contentText].filter(Boolean).join('  ·  ') || header
  } else {
    ...
    <div className="group/row">
      <div
        ...
        className={`flex items-start gap-2.5 py-1.5 text-[14px] leading-[1.5] text-[var(--muted-foreground)] ${
          canToggle ? 'cursor-pointer transition-colors hover:text-[var(--foreground)]' : ''
        }`}
      >
        ...
        <div className="min-w-0 flex-1">
          {showChatBody ? (
            <div className="space-y-1.5 italic leading-[1.6]">
              {thoughtText ? <MarkdownRenderer content={thoughtText} variant="trace" /> : null}
              {contentText ? <MarkdownRenderer content={contentText} variant="trace" /> : null}
            </div>
          ) : isThinking ? (
            ...
            <>
              <span className={`block font-medium ${active ? 'dt-breathing-text' : ''}`}>
                {headline}
              </span>
              {thoughtText || contentText ? (
                <div className="mt-1 leading-[1.6]">
                  <MarkdownRenderer content={thoughtText || contentText} variant="trace" />
                </div>
              ) : null}
            </>
          ) : (
                    ...
                    return (
                      <div key={trace.callId} className="mt-1.5 pl-0.5">
                        ...
                        <div className="mt-0.5 space-y-0.5">
                          ...
                          {(trRole === 'retrieve' || trKind === 'math_render_output') &&
                            rawEvts.length > 0 && (
...
export function StreamingStatus({
  ...
  const textColor =
    mode === 'responded' ? 'text-[var(--muted-foreground)]/70' : 'text-[var(--muted-foreground)]'
  const Mark =
    mode === 'tool_using'
      ? ToolMark
 ...
 *
 * The trace is expanded by default, including after the final answer lands.
 * The header remains a disclosure toggle so the user can fold or re-open it.
 */
