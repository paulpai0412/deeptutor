...
function BulletLine({ bulletClass, children }: { bulletClass: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-2 leading-[1.65] text-[var(--foreground)]">
      <span className={`mt-[3px] shrink-0 text-[10px] leading-[1.5] ${bulletClass}`}>●</span>
...
function ToolResultBlock({ text }: { text: string }) {
  ...
  const tr = (en: string, zh: string, zhTW: string) =>
    selectLocalizedText(i18n.language, { en, zh, zhTW })
    ...
    <div className="ml-4 rounded-md border border-[var(--border)]/60 bg-[var(--muted)]/50 px-2.5 py-1.5">
      ...
      {overflow && (
        ...
        >
          {open
            ? tr('collapse', '收起', '收起')
            : tr(`… +${hidden} lines`, `… 展开 +${hidden} 行`, `… 展開 +${hidden} 行`)}
        </button>
      )}
    </div>
  )
