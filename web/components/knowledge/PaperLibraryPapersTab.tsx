...
const EXAM_READY_STATUSES = new Set(['ready', 'ready_with_warnings', 'partial'])
...
function buildPaperTree(folders: string[], papers: PaperLibraryRecord[]): PaperTreeNode[] {
...
export default function PaperLibraryPapersTab({
  ...
  const [selectedFolderPath, setSelectedFolderPath] = useState(searchParams.get('folder') ?? '')
  const [query, setQuery] = useState('')
  const [search, setSearch] = useState('')
  const [status, setStatus] = useState('')
  const [loading, setLoading] = useState(true)
  const [errorMsg, setErrorMsg] = useState<string | null>(null)
  const [selectedPaperId, setSelectedPaperId] = useState<string | null>(searchParams.get('paper'))
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setExpandedFolders(previous => (previous.size ? previous : new Set(contents.folders)))
    } catch (error) {
  ...
  const handleRename = useCallback(async () => {
    ...
    try {
      const updated = await renameLibraryPaper(libraryId, editing.id, editing.name.trim())
      setPapers(previous =>
  ...
  const handleMove = useCallback(
    async (paperId: string, targetLibraryId: string, targetFolderPath = '') => {
      setErrorMsg(null)
      try {
        const moved = await movePaper(libraryId, paperId, targetLibraryId, targetFolderPath)
        if (targetLibraryId !== libraryId) {
          ...
          setPapers(previous =>
            previous.map(paper => (paper.paper_id === moved.paper_id ? moved : paper))
          )
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[var(--background)] sm:flex-row">
      <PaperList
        ...
        onCreateFolder={(name, parentPath) => void handleCreateFolder(name, parentPath)}
...
function PaperList({
  ...
  onCancelRename: () => void
  onMove: (paperId: string, libraryId: string, folderPath?: string) => void
  ...
  const movePaperTo = (paperId: string, targetLibraryId: string, targetFolderPath = '') => {
    setMoveMenuFor(null)
    setDropTarget(null)
    setDragPath(null)
    onMove(paperId, targetLibraryId, targetFolderPath)
      ...
      return (
        <li key={`folder:${node.path}`}>
          <div
            ...
            onDragLeave={() => setDropTarget(current => (current === node.path ? null : current))}
            onDrop={event => {
              event.preventDefault()
              event.stopPropagation()
        ...
        {isEditing ? (
          ...
          >
            <input
              autoFocus
              value={editing.name}
              onChange={event => onStartRename(paper.paper_id, event.target.value)}
          ...
          >
            ...
            >
              ...
              {paper.status !== 'ready_with_warnings' && (
                <span
                  className={`shrink-0 rounded-full px-1 py-0.5 text-[8px] ${statusClass(paper.status)}`}
                >
            ...
            <div className="flex shrink-0 items-center opacity-0 transition-opacity group-hover/row:opacity-100">
              <button
                type="button"
                onClick={() =>
                  setMoveMenuFor(current => (current === paper.paper_id ? null : paper.paper_id))
                }
                disabled={!canMove}
                title={t('Move to…')}
              ...
              <button
                ...
                title={canMove ? t('Delete file') : t('Cannot delete while processing')}
    ...
    <aside className="flex h-full max-h-[360px] w-full shrink-0 flex-col border-b border-[var(--border)] bg-[var(--card)]/40 sm:max-h-none sm:w-[220px] sm:border-b-0 sm:border-r">
      <div className="flex items-center justify-between gap-1 px-2.5 pb-1.5 pt-2.5">
        <div className="flex min-w-0 items-center gap-1.5">
          <span className="text-[12px] font-medium text-[var(--foreground)]">{t('Files')}</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-1.5 pb-2.5">
        {errorMsg && (
          <div
            role="alert"
            className="rounded-md border border-red-200 bg-red-50 px-2.5 py-2 text-[11px] text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300"
          >
            {errorMsg}
          </div>
        )}
        ...
        {loading && !papers.length ? (
          <div className="space-y-1">
            {[0, 1, 2].map(item => (
              <div key={item} className="h-8 animate-pulse rounded-md bg-[var(--muted)]/40" />
            ))}
          </div>
        ) : !papers.length ? (
...
function PaperPreview({
  ...
  const source = useMemo<FilePreviewSource | null>(() => {
    ...
    return {
      ...
      mimeType: paper.original_filename.toLowerCase().endsWith('.doc')
        ? 'application/msword'
        : 'application/pdf',
